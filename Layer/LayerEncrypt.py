...
def layer_encrypt(input_file, keyID):
    ...
    main_key_file_path = 'fcf1776bb451c703.bin'
    # 读取主密钥和盐值的二进制文件
    ...
    if salt is not None and encrypted_file is not None:
        ...
        if hmac_value is not None:
            print(encrypted_file, iv, hmac_value)
            return salt + keyID.encode('utf-8') + encrypted_file + iv + hmac_value
    return None

if __name__ == '__main__':
    keyID = 'fcf1776bb451c703'
    cipher = layer_encrypt(load_binary_file('testfile.txt'), keyID)
